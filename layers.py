...
class FeatureEmbeddingDict(nn.Module):
    def __init__(self, 
        ...
        self.pretrained_codes = pretrained_codes  # 保存参数
        ...
        self.embedding_layers['label'] = nn.Embedding(2, feat_dim)
    ...
    def dict2tensor(self, embedding_dict, flatten_emb=False, feature_list=[], feature_source=[], feature_type=[]):
        ...
        if 'label' in embedding_dict.keys():
            feature_emb_list.append(embedding_dict['label'])
        if flatten_emb:
        ...
        for feature in inputs.keys():
            if feature == 'label':
                ...
                if feature_spec["type"] == "numeric":
                    ...
                    if self.pretrained_codes and feature in self.pretrained_codes:
                        # 获取当前特征的pretrained codes（已经在显存中）
                        ...
                        # # 平均池化
                        # embeddings = torch.stack(code_embeddings, dim=2).mean(dim=2)
                        # 拼接
